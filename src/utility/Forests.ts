  ...
  private createGeometry(): BufferGeometry {
    ...
    const positions = flatMap(this._tiles, (tile, j) => {
      ...
      for (let i = 0; i < treesPerForest; i++) {
        ...
        const localPos = randomPointOnCoastTile(
          waterAdjacency(this._globalGrid, tile),
          mapScale
        );
    ...
    const params = flatMap(this._tiles, (tile) => {
      const spriteIndex = () =>
        //@ts-ignore
        tile.treeIndex * cols + Math.floor(Math.random() * cols);
      //@ts-ignore
      const treesPerForest = this.numTreesPerForest(tile.treeIndex);
      //@ts-ignore
    ...
    const buffer = new Float32Array(params.length * 3);

    for (let i = 0; i < params.length; i++) {
      buffer[i * 3 + 0] = params[i].x;
      buffer[i * 3 + 1] = params[i].y;
      buffer[i * 3 + 2] = params[i].z;
    }
