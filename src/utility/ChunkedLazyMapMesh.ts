...
export default class ChunkedLazyMapMesh
  extends Object3D
  implements TileDataSource
{
  ...
  constructor(
    ...
    private options: MapMeshOptions
  ) {
    super();
    ...
    const chunkSize = Math.min(
      (tileGrid.width * tileGrid.height) / 4,
      Math.pow(32, 2)
    );
    ...
    const chunks: TileData[][][] = range(numChunksX).map((x) =>
      range(numChunksY).map((_) => [])
    );
    ...
    this.quadtree = new QuadTree<MapThunk>(this.thunks, 1, (thunk: MapThunk) =>
      thunk.computeCenter()
    );
  }
  ...
  updateVisibility(camera: Camera) {
    ...
    this.quadtree
      .queryRange(boundingBox)
      .forEach((thunk) => thunk.updateVisibility(true));
  }
...
class MapThunk extends Object3D implements TileDataSource {
  ...
  computeCenter(): Vector2 {
    ...
    sphere.setFromPoints(
      this.tiles.map(
        (tile) =>
          new Vector3(qrToWorldX(tile.q, tile.r), qrToWorldY(tile.q, tile.r))
      )
    );
  ...
  load() {
    if (!this._loaded) {
      this._loaded = true;
      const mesh = (this.mesh = new MapMesh(
        this.tiles,
        this.options,
        this.grid
      ));
