...
export default class MapMesh extends Group implements TileDataSource {
  ...
  constructor(
    ...
    options.hillsNormalTexture.wrapS = options.hillsNormalTexture.wrapT =
      RepeatWrapping;
    options.terrainAtlasTexture.wrapS = options.terrainAtlasTexture.wrapT =
      RepeatWrapping;
    options.undiscoveredTexture.wrapS = options.undiscoveredTexture.wrapT =
      RepeatWrapping;
  ...
  private async createLandMesh(tiles: MapMeshTile[]) {
    ...
    const geometry = createHexagonTilesGeometry(
      tiles,
      this.globalGrid,
      0,
      this.options
...
function createHexagonTilesGeometry(
  tiles: MapMeshTile[],
  grid: Grid<TileData>,
