...
export function createHexagon(
  ...
  var positions = new Float32Array(numFaces * 3 * 3),
    p = 0;
  var texcoords = new Float32Array(numFaces * 3 * 2),
    t = 0;
  var border = new Float32Array(numFaces * 3 * 1),
    e = 0;
...
export function randomPointInHexagon(hexRadius: number): Vector3 {
  ...
  var r = Math.random(),
    s = Math.random();
  var rSqrt = Math.sqrt(r),
    sSqrt = Math.sqrt(s);

  return A.clone()
    .multiplyScalar(1 - rSqrt)
 ...
 * Returns a random point in the regular hexagon at (0,0) with given hex radius on the Z=0 plane.
 */
export function randomPointInHexagonEx(
  hexRadius: number,
  modifier: (cornerIndex: number) => number
): Vector3 {
  // the hexagon consists of 6 triangles, construct one of them randomly
  var startCornerIndex = Math.floor(Math.random() * 6);
