...
class Animation {
  ...
  constructor(
    ...
    private easingFunction = Animation.easeInOutQuad
  ) {}
...
export default class Controller implements MapViewController {
  ...
  onKeyDown = (e: KeyboardEvent) => {
    if (e.keyCode == 32) {
      // SPACE BAR
      console.log(
        `center view on QR(${this.selectedQR.q},${this.selectedQR.r})`
      );
  ...
  onMouseDown = (e: MouseEvent) => {
    // TODO original
    this.pickingCamera = this.controls.getCamera().clone() as Camera;
    // TODO updated
    // this.pickingCamera = this.controls.getCamera();
    // TODO other options is to clone using Object assing
    // const originalCamera = this.controls.getCamera();
    // const newCamera = new Camera();
    // Object.assign(newCamera, originalCamera);
    // this.pickingCamera = newCamera;
  ...
  onMouseMove = (e: MouseEvent) => {
    ...
    if (this.mouseDownPos) {
      ...
      const dv = (this.lastDrag = mousePos
    ...
    if (window.innerHeight == screen.height && !this.mouseDownPos) {
      ...
      const screenCenter2D = new Vector2(
        window.innerWidth / 2,
      ...
      if (
        Math.abs(diff.x) > screenCenter2D.x - scrollZoneSize ||
        Math.abs(diff.y) > screenCenter2D.y - scrollZoneSize
      ) {
  ...
  showDebugInfo() {
    ...
    this.debugText.innerHTML = `Selected Tile: QR(${tileQR.q}, ${tileQR.r}), 
            ...
            &nbsp; &bull; &nbsp; Camera Looks At (Center): XYZ(${camPos.x.toFixed(
              2
            )}, ${camPos.y.toFixed(2)}, ${camPos.z.toFixed(2)})`;
  }
