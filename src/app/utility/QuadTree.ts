...
export default class QuadTree<T> {
  ...
  constructor(
    ...
    if (bounds) {
      ...
      const min = data.reduce(
        (min: Vector2, item: T) => min.min(pos(item)),
        pos(data[0])
      );
      const max = data.reduce(
        (max: Vector2, item: T) => max.max(pos(item)),
        pos(data[0])
      );
  ...
  queryRange(range: BoundingBox): T[] {
    ...
    pointsInRange = this.data
      ? this.data.filter((item: T) => range.containsPoint(this.pos(item)))
      : [];
    if (this.isLeaf()) {
      return pointsInRange;
    }
